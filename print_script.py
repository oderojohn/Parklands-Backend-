import requests
import json
import argparse
import os
from datetime import datetime
from escpos.printer import Network
import time
import re
import sys

class LocalPrinter:
    def __init__(self, ip=None, port=None):
        self.PRINTER_IP = ip or os.getenv('PRINTER_IP', "192.168.10.175")
        self.PRINTER_PORT = int(port or os.getenv('PRINTER_PORT', 9100))
        self.MAX_RETRIES = 3
        self.RETRY_DELAY = 2
        self.bw_image_path = r"C:\ReceptionIQ\python Backend\myproject\myapp\logo-dark.bmp"
        # Predefined size commands

        self.zes = [
            ('1x1', b'\x1D\x21\x00'),
            ('2x1', b'\x1D\x21\x10'),
            ('1x2', b'\x1D\x21\x01'),
            ('2x2', b'\x1D\x21\x11'),
            ('3x3', b'\x1D\x21\x22')
        ]

    def _mask_phone(self, phone):
        """Mask phone numbers to show first 4 and last 2 digits (0792******01)"""
        if not phone:
            return ""

        # Remove any non-digit characters
        digits = re.sub(r'\D', '', phone)

        if len(digits) >= 6:
            return f"{digits[:4]}******{digits[-2:]}"
        return phone  # Return original if too short to mask

    def _get_printer(self):
        """Establish connection to printer with retry logic"""
        for attempt in range(self.MAX_RETRIES):
            try:
                printer = Network(self.PRINTER_IP, self.PRINTER_PORT)
                return printer
            except Exception as e:
                if attempt == self.MAX_RETRIES - 1:
                    raise
                print(f"Connection attempt {attempt + 1} failed: {str(e)} - retrying...")
                time.sleep(self.RETRY_DELAY)
        return None

    def _set_size(self, printer, size_name):
        """Set text size using predefined zes commands"""
        for name, cmd in self.zes:
            if name == size_name:
                printer._raw(cmd)
                return
        printer._raw(self.zes[0][1])  # Default to 1x1 if not found

    def _print_common_header(self, printer, title, code):
        """Print common header for both receipt types"""
        try:
            printer.image(self.bw_image_path)
        except Exception as img_error:
            print(f"Couldn't print image: {img_error}")

        # Set larger size for code
        self._set_size(printer, '2x2')
        printer.set(align='center', bold=True)
        printer.text(f"{code}\n")
        self._set_size(printer, '1x1')  # Reset size

        printer.set(align='left', bold=False)
        printer.text("\n------------------------------------------\n")

    def print_package_receipt(self, package_data):
        """Print the package label receipt with enhanced styling"""
        printer = None
        try:
            printer = self._get_printer()
            if not printer:
                return False

            # Mask phone numbers before printing
            package_data['recipient_phone'] = self._mask_phone(package_data.get('recipient_phone', ''))
            package_data['dropper_phone'] = self._mask_phone(package_data.get('dropper_phone', ''))

            # Capitalize recipient name
            recipient_name = package_data['recipient_name'].upper()

            self._print_common_header(printer, "PACKAGE RECEIPT", recipient_name)

            # Extra large recipient name
            self._set_size(printer, '3x3')
            printer.set(align='center', bold=True)
            printer.text(recipient_name + "\n\n")
            self._set_size(printer, '1x1')  # Reset size

            printer.set(bold=True)
            printer.text("PACKAGE DETAILS\n")
            printer.set(bold=False)
            printer.text("-----------------------------\n")

            printer.text(f"Type: {package_data['type']}\n")
            printer.text(f"Code: {package_data['code']}\n")
            printer.text(f"Description: {package_data['description']}\n")
            if package_data.get('shelf'):
                printer.text(f"Shelf: {package_data['shelf']}\n")

            printer.text("\n")

            # Medium size for section headers
            self._set_size(printer, '2x1')
            printer.set(align='center', bold=True)
            printer.text("RECIPIENT INFORMATION\n")
            self._set_size(printer, '1x1')

            printer.set(align='left', bold=True)
            printer.text(f"Name: {recipient_name}\n")
            printer.set(bold=False)
            printer.text(f"Phone: {package_data['recipient_phone']}\n")

            printer.text("\n")
            printer.set(bold=True)
            printer.text("DELIVERY INFORMATION\n")
            printer.set(bold=False)
            printer.text(f"Dropped by: {package_data['dropped_by']}\n")
            printer.text(f"Phone: {package_data['dropper_phone']}\n")

            printer.text("\n")
            printer.text(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
            if 'created_by' in package_data:
                printer.text(f"Processed by: {package_data['created_by']}\n")

            printer.text("\n")
            printer.set(align='center', bold=True)
            printer.text("Thank you for using\nPSC Package Service\n")
            printer.set(bold=False)
            printer.text("Handled by PSC Security\n")
            printer.text("Designed by PSC ICT\n\n")
            printer.cut()
            return True
        except Exception as e:
            print(f"Label print failed: {e}")
            return False
        finally:
            if printer:
                printer.close()

def authenticate(api_base, username, password):
    """Authenticate and get JWT token"""
    url = f"{api_base}/api/auth/login/"
    data = {'username': username, 'password': password}
    response = requests.post(url, json=data)
    if response.status_code == 200:
        return response.json()['access']
    else:
        raise Exception(f"Authentication failed: {response.text}")

def get_package_data(api_base, token, package_code):
    """Get package data by code"""
    url = f"{api_base}/api/packages/"
    headers = {'Authorization': f'Bearer {token}'}
    params = {'code': package_code}
    response = requests.get(url, headers=headers, params=params)
    if response.status_code == 200:
        data = response.json()
        if data['results']:
            return data['results'][0]
        else:
            raise Exception("Package not found")
    else:
        raise Exception(f"Failed to get package: {response.text}")

def get_unprinted_packages(api_base, token):
    """Get all pending packages that haven't been printed yet"""
    url = f"{api_base}/api/packages/"
    headers = {'Authorization': f'Bearer {token}'}
    params = {'status': 'pending'}
    response = requests.get(url, headers=headers, params=params)
    if response.status_code == 200:
        data = response.json()
        # Filter packages that haven't been printed (if printed field exists)
        packages = data['results']
        unprinted_packages = []
        for package in packages:
            if 'printed' not in package or not package['printed']:
                unprinted_packages.append(package)
        return unprinted_packages
    elif response.status_code == 401:
        raise Exception("Token expired")
    else:
        raise Exception(f"Failed to get unprinted packages: {response.text}")

def update_package_printed(api_base, token, package_id):
    """Update package printed status to True"""
    url = f"{api_base}/api/packages/{package_id}/"
    headers = {'Authorization': f'Bearer {token}'}
    data = {'printed': True}
    response = requests.patch(url, headers=headers, json=data)
    if response.status_code not in [200, 201]:
        raise Exception(f"Failed to update package printed status: {response.text}")

def main():
    parser = argparse.ArgumentParser(description='Automatically print receipts for unprinted packages from cloud API')
    parser.add_argument('--type', default=os.getenv('PRINT_TYPE', 'package'), choices=['package'], help='Type of item to print')
    parser.add_argument('--api-base', default=os.getenv('API_BASE_URL', 'http://127.0.0.1:8005'), help='Cloud API base URL')
    parser.add_argument('--username', default=os.getenv('API_USERNAME'), help='API username')
    parser.add_argument('--password', default=os.getenv('API_PASSWORD'), help='API password')
    parser.add_argument('--printer-ip', help='Printer IP')
    parser.add_argument('--printer-port', type=int, help='Printer port')
    parser.add_argument('--interval', type=int, default=1, help='Check interval in seconds (default: 1)')

    args = parser.parse_args()

    # Prompt for required fields if not provided
    if not args.username:
        args.username = input("Enter API username: ").strip()
        if not args.username:
            parser.error("--username is required")
    if not args.password:
        import getpass
        args.password = getpass.getpass("Enter API password: ").strip()
        if not args.password:
            parser.error("--password is required")

    try:
        # Authenticate once
        token = authenticate(args.api_base, args.username, args.password)
        print("Authenticated successfully")

        printer = LocalPrinter(ip=args.printer_ip, port=args.printer_port)

        while True:
            try:
                # Get unprinted packages
                unprinted_packages = get_unprinted_packages(args.api_base, token)
                for package in unprinted_packages:
                    print(f"Printing receipt for package {package['code']}")
                    # Print
                    success = printer.print_package_receipt(package)
                    if success:
                        print("Receipt printed successfully")
                        # Update printed status
                        update_success = update_package_printed(args.api_base, token, package['id'])
                        if update_success:
                            print(f"Updated printed status for package {package['code']}")
                        else:
                            print(f"Failed to update printed status for package {package['code']}")
                    else:
                        print("Failed to print receipt")

            except Exception as e:
                print(f"Error in loop: {e}")
                # Continue to next iteration instead of breaking the loop
                continue

            # Wait for the specified interval
            time.sleep(args.interval)

    except Exception as e:
        print(f"Error: {e}")

if __name__ == "__main__":
    main()